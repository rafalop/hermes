...
//go:generate go run github.com/cilium/ebpf/cmd/bpf2go -type blk_req_event -target $BPF_ARCH -cc $BPF_CLANG -cflags $BPF_CFLAGS bpf io_latency.c -- -I../header
...
// request ops
// copied from vmlinux.h 8 lsb for op, 24 msb for flags
const (
	REQ_OP_READ  = 0
	REQ_OP_WRITE = 1
	REQ_OP_FLUSH = 2
...
func (loader *IoLatLoader) getBlkLatRecs() (blkRecs []BlkLatRec) {
	...
	for { // may need to be bounded
		...
		if err := binary.Read(bytes.NewBuffer(record.RawSample), binary.LittleEndian, &blkEvent); err != nil {
			if err == io.EOF {
				break
			}
