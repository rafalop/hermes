...
func NewIoLatExporter(sourceDir string) *IoLatExporter {
		logrus.Info("creating new iolat exporter")
...
func (i *IoLatExporter) Collect(ch chan<- prometheus.Metric) {
	bytes, err := GatherDataAsBytes(i.viewDir, IO)
	if err != nil {
	...
	for dev, rec := range data.PerDev {
		...
		//i.totalIos.With(labels).Set(float64(rec.Reads))
	}
